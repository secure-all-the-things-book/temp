...
public class AuthApplication {
    ...
    InMemoryUserDetailsManager inMemoryUserDetailsManager(PasswordEncoder pw) {
        return new InMemoryUserDetailsManager(User.withUsername("josh").password(pw.encode("pw"))
                .authorities("USER")
                .build(),
                User.withUsername("rob").password(pw.encode("pw"))
    ...
    SecurityFilterChain springSecurity(HttpSecurity http) {
        ...
        http
                .authorizeHttpRequests(x -> x
                        .requestMatchers("/oauth2/authorize").access(adm.authenticated())
                        .anyRequest().authenticated()
                )
                .formLogin(Customizer.withDefaults())
                .with(mcpAuthorizationServer(), Customizer.withDefaults())
                .oauth2AuthorizationServer(c -> c
